...
//worker send mail
$beanstalk->addWorker('mailWorker', function (Job $job) use($config,$di){
    $worker=WorkerProviderFactory::create('mailWorker');
    $worker->addWorker($config,$di,$job);
    // It's very important to send the right exit code!
    exit(0);
});

//worker send sms
//$beanstalk->addWorker('smsWorker', function (Job $job) use($config,$di){
//    $worker=WorkerProviderFactory::create('smsWorker');
//    $worker->addWorker($config,$di,$job);
//    // It's very important to send the right exit code!
//    exit(0);
//});
