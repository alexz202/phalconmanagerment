...
if (empty($_FILES) === false) {
    ...
    $remote=intval($config['remote']);
    if($remote==1){
        $source=new \Zejicrm\Source($config);
        $_file= new \CURLFile(realpath($file_path));
        $params=array(
            'filename'=>$_file
        );
        $makeThumb=0;
        $inf='upload_pic_inf';
        $_data=$source->uploadFile($params,$inf,0,$save_url,$makeThumb);
        if($_data)
           $data= json_decode($_data,true);
    }

	$json = new Services_JSON();
	echo $json->encode(array('error' => 0, 'url' => $file_url,'data'=>$data));
	exit;
}
