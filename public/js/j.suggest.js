	(function($) {
		...
		$.suggest = function(input, options) {
			...
			if($.trim($input.val())=='' || $.trim($input.val())=='中文') $input.val('中文').css('color','#aaa');
			...
			function resetPosition() {
                ...
                var diff=0;
				if($results.height()>=0) diff=298-$results.height();
				//console.log('height'+$results.height());
				if(allheight-offset.top<=380){
                    $results.css({
                        top: (offset.top + input.offsetHeight-330+diff) + 'px',
                        left: offset.left + 'px'
                    });
				}else{
                    $results.css({
